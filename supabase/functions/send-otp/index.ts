...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const skipRateLimit = email === 'reggie@storymode.co';

    if (!skipRateLimit) {
      // Check rate limiting (loosened)
      ...
      const captchaThreshold = 7; // previously 5
      const lockThreshold = 12;   // only lock after 12 attempts
      const lockMinutes = 5;      // shorter lock
      ...
      if (rateLimit) {
        ...
        if (new Date(rateLimit.updated_at) < fifteenMinutesAgo) {
          ...
          const needsCaptcha = newCount >= captchaThreshold;
          
          // If over lock threshold with no CAPTCHA, apply short lock
          if (newCount >= lockThreshold && !captchaToken) {
            await supabase
              .from("auth_rate_limits")
              .update({ 
                attempt_count: newCount, 
                locked_until: new Date(now.getTime() + lockMinutes * 60 * 1000).toISOString()
              })
              .eq("id", rateLimit.id);

            return new Response(
              JSON.stringify({ 
                error: "Too many attempts. Please wait a few minutes or complete the CAPTCHA.",
                requiresCaptcha: true,
                retryAfter: lockMinutes
              }),
              { status: 429, headers: { ...corsHeaders, "Content-Type": "application/json" } }
            );
          }
